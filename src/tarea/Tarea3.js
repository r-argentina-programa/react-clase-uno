...
const [isChecked, setIsChecked] = React.useState(props.initialValue);
  ...
  return (
    <>
      <input name={props.name} type='checkbox' checked={isChecked} onChange={() => setIsChecked(!isChecked)}/><span>{props.name}</span>
    </>
  )
}
...
export function CheckboxList(props) {
  ...
  return (
   ...
   <UncontrolledCheckbox key={item} name={item} initialValue={value}/>)
  )
}
